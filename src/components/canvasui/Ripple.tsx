...
export interface RippleOptions {
  ...
  interval?: number;
  /** Where ambient ripples spawn: random across the field, or a fixed point. */
  ambientOrigin?: "random" | "center";
  /** Horizontal spawn ratio for center ambient (0–1). Default 0.5. */
  ambientX?: number;
  /** Vertical spawn ratio for center ambient (0–1). Default 0.5. */
  ambientY?: number;
}
...
export function createRipple(
  ...
  function spawnAmbient() {
    ...
    if (w < 10 || h < 10) return;
    if (config.ambientOrigin === "center") {
      const x = w * Math.min(Math.max(config.ambientX, 0), 1);
      const y = h * Math.min(Math.max(config.ambientY, 0), 1);
      splash(x, y, 0.95);
      return;
    }
    splash(
  ...
  wake = start;
  start();
  if (config.interval > 0 && !reducedMotion) spawnAmbient();
