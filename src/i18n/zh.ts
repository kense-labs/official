...
export const zh: Messages = {
  ...
  solutions: {
    ...
    items: [
      {
        id: 'monitor',
        name: 'Kense Monitor',
        tagline: '外面的世界变了，计划还停在昨天',
        description: '把冲突、制裁、航运和宏观信号收成一条可问、可订、可接入现有系统的证据流。',
      ...
      },
      {
        id: 'finance',
        name: 'Kense Finance',
        tagline: '同一套行情，研究台和交易台一起用',
        description: '股票、宏观、衍生品和资讯接一次，Python、工作台、MCP、REST 读同一份。',
      ...
      },
      {
        id: 'science',
        name: 'Kense Science',
        tagline: '论文、Notebook 和实验记录放在一起',
        description: '本地工作区里跑分析、留轨迹。别人打开，就能看见结果是怎么来的。',
      ...
      },
      {
        id: 'insight',
        name: 'Kense Insight',
        tagline: '指标只定义一次，各处读同一个数',
        description: '用代码写清口径和权限。BI、产品和自动化都走 SQL / API / MCP。',
    ...
    monitor: {
      ...
      lead: '冲突、制裁、航运、宏观收成一条流。能问、能订、能接到现在用的系统。',
      stats: [
        { value: '4', label: '信号域' },
        { value: '只读', label: '不写业务系统' },
        { value: '可追', label: '条条带来源' },
      ],
      capabilities: {
        kicker: '能力',
        title: '外面变了，里面要知道',
        body: 'Monitor 给出证据。任务在 Work 里开。写回走 OS。',
        items: [
          { title: '问', body: '用一句话查当前态势，并落到标的、站点或订单。' },
          { title: '订', body: '定时摘要、关键事件告警，以及下游能吃的事件流。' },
          { title: '接', body: 'Widget、MCP、API。现有工作台把态势当结构化输入。' },
          { title: '只读', body: 'Monitor 不改库存、持仓或排产。缺数据不当成零风险。' },
          { title: '可核验', body: '每条事件带着来源和时间。事后能打开，而不是靠聊天记录。' },
        ],
      },
      uses: {
        kicker: '用法',
        title: '同一条流，两种班',
        items: [
          { title: '研究台', body: '外部冲击带着来源出现在研究台，写成假设，而不是写成传闻。' },
          { title: '计划', body: '航运、制裁、原料集中度进入交期和缓冲，而不是会后再补。' },
          { title: '合规', body: '名单变更进订阅。该停的停，依据留得住。' },
        ],
      },
      artifact: {
        kicker: '结构',
        title: '证据在这。动作在别处。',
        body: 'Monitor 只出态势。人在 Work 里判断。Bot 要改系统，必须过 OS。',
        inLabel: '信号',
        outLabel: '用法',
        points: [
          '冲突、制裁、海事、宏观，只读接入',
      ...
      close: {
        title: '让计划和外面的世界对齐。',
        body: '免费开始，或约一次演示，走完一条从信号到决策的路径。',
      },
    },
    finance: {
      documentTitle: 'Kense Finance — Kense',
      lead: '股票、宏观、期权、资讯接一次。Python、工作台、MCP、REST 读同一份。',
      stats: [
        { value: '1', label: '次接入' },
        { value: '4', label: '消费端' },
        { value: '0', label: '第二套行情' },
      ...
      capabilities: {
        kicker: '能力',
        title: '一份行情，四处用',
        body: '规范化一次。分析师、量化、工具不再各接各的。',
        items: [
          { title: '股票与 ETF', body: '行情、基本面、估值、持仓、成分。研究和回测读同一字段。' },
          { title: '宏观与固收', body: '日历、利率、曲线、利差，能和价格对齐。' },
          { title: '期权与期货', body: '期权链、希腊值、期货。波动和对冲不用另接一套。' },
          { title: '资讯与披露', body: '新闻和监管文件带着时间戳和链接，能和价格并排看。' },
          { title: '同一契约', body: 'SDK、工作台、MCP、REST。权限和 schema 不各写各的。' },
        ],
      },
      uses: {
        kicker: '用法',
        title: '研究台和量化台，同一本书',
        items: [
          { title: '研究', body: '价格和披露并排。导出时带来源。外面的世界变了，就去问 Monitor。' },
          { title: '量化', body: 'Python 加载历史和基本面。回测能复现，因为字段没被私下改过。' },
          { title: '工具', body: '经 MCP 取链和摘要。配额和 ACL 按租户走。下单仍须审批。' },
        ],
      },
      artifact: {
        kicker: '结构',
        title: '一次规范化，多端读。',
        body: 'Finance 管数据和缓存。Work 是任务入口。要动账户，走 OS。',
        inLabel: '源',
        outLabel: '端',
        points: [
          '公有、授权、内部源，一次洗成同一份',
          '每条序列留 provider 和抓取时间',
          '查询默认只读',
        ],
      },
      close: {
        title: '研究台和交易台，用同一份行情。',
        body: '免费开始，或约一次演示，接进第一批股票和资讯。',
      },
    },
    science: {
      documentTitle: 'Kense Science — Kense',
      lead: '论文、Notebook、实验记录放在同一个本地工作区。别人打开，能看见结果是怎么来的。',
      stats: [
        { value: '本地', label: '默认不出域' },
        { value: '3', label: '文献入口' },
        { value: '可回放', label: '带着执行轨迹' },
      ],
      capabilities: {
        kicker: '能力',
        title: '结果和跑出结果的过程，放一起',
        body: '文献、内核、文件、连接器在同一个项目里。审阅时不用猜。',
        items: [
          { title: '文献', body: 'DOI、PMID、arXiv 导入。开放全文、PDF 图、引用格式。' },
          { title: 'Python / R', body: '持久内核。本机、SSH 或 Slurm。后台跑，轨迹留着。' },
          { title: '版本', body: '代码、输入、环境、跑次，绑在一个校验和上。' },
          { title: '连接器', body: '研究技能和 MCP。没授权的源，不会被悄悄调用。' },
          { title: '本地优先', body: '项目和凭据在磁盘上。出工作区，才过 OS。' },
        ],
      },
      uses: {
        kicker: '用法',
        title: '复现，或做综述',
        items: [
          { title: '复现', body: '论文、数据和阈值放在一起。Notebook 跑完，表和图能对上。' },
          { title: '综述', body: '文库共享。抽 PDF。导出的包，换一台机器也能打开。' },
          { title: '作业', body: 'HPC 跑完回来，仍然看得到调用过什么。' },
        ],
      },
      artifact: {
        kicker: '结构',
        title: '先在这台机器上跑完。',
        body: 'Science 把问题和产物留在本地。要出域，Box 和 Sentry 才上场。',
        inLabel: '材料',
        outLabel: '去向',
        points: [
          '工具活动和内核参数，跟报告一起可见',
      ...
      close: {
        title: '把结果和跑出它的那次实验放一起。',
        body: '免费开始，或约一次演示，走完从问题到回放。',
      },
    },
    insight: {
      documentTitle: 'Kense Insight — Kense',
      lead: '指标用代码写死。BI、产品、自动化走 SQL / API / MCP，读同一个数。',
      stats: [
        { value: '1', label: '次定义' },
        { value: '4', label: '接口' },
        { value: '0', label: '表外口径' },
      ...
      capabilities: {
        kicker: '能力',
        title: '口径只写一次',
        body: '定义、权限、缓存在运行时生效。提示词改不掉行级规则。',
        items: [
          { title: '建模', body: '指标、维度、关联、粒度写成代码，环境之间能搬。' },
          { title: '权限', body: '身份映射到行和租户。SQL 发出去之前就套上。' },
          { title: '缓存', body: '同样的问法共用缓冲。预聚合扛住追问。' },
          { title: 'SQL / API', body: 'Postgres 兼容、REST、GraphQL。查的是指标，不是底表。' },
          { title: 'MCP', body: '工具只能选已经定义的度量，不能对原始表自己编 SQL。' },
        ],
      },
      uses: {
        kicker: '用法',
        title: '图、产品和 Bot，同一个数',
        items: [
          { title: 'BI 和产品', body: '分析师用 SQL 探。产品用 REST 嵌。收入这两个字只指一件事。' },
          { title: '自动化', body: 'Meta API 列出能问的度量。MCP 选这些，不选裸表。' },
          { title: '成本', body: '交互和追问打在缓存上，不打穿数仓。' },
        ],
      },
      artifact: {
        kicker: '结构',
        title: '一次定义，缓存，多端出。',
        body: 'Insight 管字典和性能。Work 用来问。答案若要触发动作，走 OS。',
        inLabel: '源',
        outLabel: '口',
        points: [
          '先对模型，再编译成源 SQL 或打中预聚合',
          '开发模式鉴权只留在本地',
          '生产用密钥、JWT、最小权限',
        ],
      },
